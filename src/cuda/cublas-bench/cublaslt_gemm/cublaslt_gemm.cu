...
float timing_matmul_tn(size_t m, size_t n, size_t k, size_t batch, int warmup, int iter) {
    ...
    int deviceX=0;
    int deviceY=1;
    cudaSetDevice(deviceX);
    cudaDeviceEnablePeerAccess(deviceY, 0);
    cudaSetDevice(deviceY);
    cudaDeviceEnablePeerAccess(deviceX, 0);

    // matrix inited on deviceY, calcuated on deviceX
    Ta *matrix_a = nullptr;
    Tb *matrix_b = nullptr;
    ...
    // init gemm
     // Set device to X to perform gemm
    cudaSetDevice(deviceX);
