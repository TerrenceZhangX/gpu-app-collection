/**
 * CUTLASS Volta TensorOp GEMM — 64×64 tile + L2-conscious swizzle.
 *
 * Usage: ./gemm_volta_64x64_swizzle <M> <N> <K>
 *
 * Combines two optimizations:
 *   1. Smaller 64×64×32 threadblock tile (4× more CTAs)
 *   2. GemmIdentityThreadblockSwizzle<8> (L2-conscious CTA mapping)
 *
 * Expected: the swizzle might help the 64×64 tile variant since it
 * generates many more CTAs that can benefit from L2-aware scheduling.
 */
...
// ---------------------------------------------------------------------------
// CUTLASS GEMM type definition (64×64 tile + Swizzle<8>)
// ---------------------------------------------------------------------------
...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    ...
    std::cerr << "  64×64 tile + Swizzle<8> GEMM" << std::endl;
    return 1;
  }
  ...
  std::cout << "=== GEMM (64×64 tile + Swizzle<8>) ===" << std::endl;
